...
{
    sealed class Watchdog1 : NetInterface
    {
        ...
        public Watchdog1(int amount_of_interfaces, Form1 form) : base(0, form)
        {
        ...
        private void Cyclic()
        {
            ...
            while (true)
            {
                ...
                if (connectedInterfaces.Count > 0)
                {
                    ...
                    {
                        if (!x.IsAvailable() && osc_info != null)
                        {
                ...
                if (toProcess.operation == Operations.DISCOVER)
                {
                    foreach (NetInterface x in interfaces)
                    {
                        if (x.IsAvailable() && !(offeredInterfacesNumbers.Contains(interfaces.IndexOf(x))))
                        {
                            ...
                            AddOfferedInterface(interfaces.IndexOf(x));
